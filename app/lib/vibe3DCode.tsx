...
export async function vibe3DCode(editor: Editor, shapeId: TLShapeId | null = null, thinkingMode: boolean = false) {
  ...
  try {
    if (thinkingMode) {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/trellis/task`, {
        method: 'POST',
        headers: {
      ...
      */
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/queue/3d`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
async function waitForCodeGeneration(taskId: string): Promise<{ content: string } | null> {
  return new Promise((resolve, reject) => {
    const eventSource = new EventSource(`${process.env.NEXT_PUBLIC_API_URL}/api/subscribe/${taskId}`);
...
async function waitForTaskResult(taskId: string): Promise<string> {
  return new Promise((resolve, reject) => {
    const eventSource = new WebSocket(`${process.env.NEXT_PUBLIC_API_URL}/api/trellis/task/ws/${taskId}`);
